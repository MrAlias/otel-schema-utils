...
func TestConverterResource(t *testing.T) {
	v0 := resource.NewWithAttributes(
		testURLV0,
		attribute.Bool("foo", true),
		attribute.Bool("untouched", true),
	)

	v3 := resource.NewWithAttributes(
		testURLV3,
		attribute.Bool("qux", true),
		attribute.Bool("untouched", true),
	)

	c := NewConverter(NewStaticClient(testSchemas))
...
func TestConverterMergeResources(t *testing.T) {
	v0 := resource.NewWithAttributes(testURLV0, attribute.Bool("foo", true))
	v1 := resource.NewWithAttributes(testURLV1, attribute.Bool("untranslated", false))
	v2 := resource.NewWithAttributes(testURLV2, attribute.Bool("untranslated", true))
